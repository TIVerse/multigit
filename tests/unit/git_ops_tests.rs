...
fn test_branch_manager_creation() {
    ...
    assert_eq!(std::mem::size_of_val(&manager), std::mem::size_of::<BranchManager>());
}
...
fn test_delete_branch() {
    ...
    // Create a branch
    manager.create_branch("to-delete", &repo.find_commit(commit).unwrap()).unwrap();
...
fn test_remote_manager_creation() {
    ...
    assert_eq!(std::mem::size_of_val(&manager), std::mem::size_of::<RemoteManager>());
}
...
fn test_remove_remote() {
    ...
    let manager = RemoteManager::new(&repo);
    manager.add("to-remove", "https://github.com/user/repo.git").unwrap();
